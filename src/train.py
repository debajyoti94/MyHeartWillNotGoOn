...
1. Obtain cleaned data using feature engineering module
    a. This module internally calls create folds to obtain
...
def train_model(dataset, fold):
    ...
    X_train = dataset_train.drop([config.output_feature,
                                  "kfold"], axis=1).values
    # print(X_train.head())
    # print(X_train.shape)
    ...
    X_valid = dataset_valid.drop([config.output_feature,
                                  "kfold"], axis=1).values
    ...
    lr = linear_model.LogisticRegression(penalty="l2", class_weight="balanced")
    ...
    p, r, f1_score, support = metrics.precision_recall_fscore_support(y_valid,
                                                                      preds)
    print(
        "---Fold={}---\nAccuracy={}\nPrecision={}\nRecall={}\nF1={}".format(
            fold, accuracy, p, r, f1_score
        )
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train", type=str, help="This parameter will simply train the model"
    )
    parser.add_argument(
        "--test",
        type=None,
